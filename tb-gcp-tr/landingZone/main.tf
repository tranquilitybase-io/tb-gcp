...
module gke-ssp {
  ...
  cluster_master_authorized_cidrs = "${concat(var.cluster_ssp_master_authorized_cidrs, list(merge(map("display_name", "initial-admin-ip"), map("cidr_block", join("", list(var.clusters_master_whitelist_ip, "/32"))))))}"
...
module gke-security {
  ...
  cluster_master_authorized_cidrs = "${concat(var.cluster_sec_master_authorized_cidrs, list(merge(map("display_name", "initial-admin-ip"), map("cidr_block", join("", list(var.clusters_master_whitelist_ip, "/32"))))))}"
...
module gke-operations {
  ...
  cluster_master_authorized_cidrs = "${concat(var.cluster_opt_master_authorized_cidrs, list(merge(map("display_name", "initial-admin-ip"), map("cidr_block", join("", list(var.clusters_master_whitelist_ip, "/32"))))))}"
