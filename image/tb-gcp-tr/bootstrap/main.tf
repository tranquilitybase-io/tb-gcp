...
module "terraform_server" {
  source = "create_bootstrap_host_module"

  project_id = "${google_project.bootstrap-res.project_id}"
  ...
  lz_branch = "${var.lz_branch}"
  service_account_email = "${google_service_account.bootstrap-sa-res.email}"
  source_cidr = []

  terraform_state_bucket_name = "${google_storage_bucket.terraform-state-bucket-res.name}"
  dependency_values = "${google_project_services.bootstrap_project_apis.services}"
}
